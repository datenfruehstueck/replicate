...
class ModelHandler {
    ...
    function collect_entries() {
        ...
        $class = 'ReplicationManagement\\'.ucfirst($this->table);
	...
	function get_entries_by_field($field, $value) {
	    $filtered_entries = [];
		foreach($this->entries as $entry) {
            if(is_subclass_of($entry->$field, 'ReplicationManagement\\AbstractModel')) {
	...
	function add($model) {
		...
		foreach($this->get_fields_from_table() as $field) {
			if(property_exists($model, $field) && $field != 'uid') {
			    if(is_subclass_of($model->$field, 'ReplicationManagement\\AbstractModel')) {
