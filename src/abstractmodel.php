...
abstract class AbstractModel {
		...
		foreach($row as $field => $value) {
			if(property_exists($this, $field)) {
                if(in_array($field, $this->config['database']['tables']) && is_numeric($value) && $value > 0) {
                    $class = 'ReplicationManagement\\'.ucfirst($field);
	...
	function update() {
		...
		foreach($this->fields as $field) {
			if(property_exists($this, $field)) {
                if($this->$field === NULL) {
                    $updates[] = '`' . $field . '` = NULL';
                } elseif(is_subclass_of($this->$field, 'ReplicationManagement\\AbstractModel')) {
    ...
    function is_the_same_as($other_model) {
        if(is_subclass_of($other_model, 'ReplicationManagement\\AbstractModel')) {
